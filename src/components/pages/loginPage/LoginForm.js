...
const FormInput = ({ type, placeholder, field }) => {
	return <input className='field' name={placeholder.toLowerCase()} type={type} placeholder={placeholder} {...field} />;
};
		...
		<Formik initialValues={initialValues} onSubmit={onSubmit} validationSchema={validationSchema}>
			{(formikProps) => (
				<FormContainer title={title} logo={logo} onSubmit={formikProps.handleSubmit}>
					...
					<button className={`btn ${buttonText === 'Sign In' ? 'btn-in' : 'btn-up'}`} type='submit'>
						{buttonText}
					</button>
				</FormContainer>
			)}
